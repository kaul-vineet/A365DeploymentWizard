...
Once the user provides the Client App ID, the agent MUST **execute these three checks itself**:

**Check 1 · Azure CLI Session:**
```
az account show --query "{Subscription:name, TenantId:tenantId, User:user.name}" -o table
```
- Show the output and ask: *"Is this the correct subscription and tenant?"*
- If failed, **execute `az login`** in async mode. Tell user: *"A browser window has opened — sign in."* Re-run after.

**Check 2 · Service Principal lookup:**
```
az ad sp show --id <client-app-id> --query "id" -o tsv
```
- If fails: Client App ID wrong or wrong tenant. Offer to re-run Check 1.
...
Once all three checks pass, proceed to Group 4.
...
After Group 4 confirmed, collect remaining parameters ONE AT A TIME:

```
🎒 Provisions confirmed. Azure verified. Permissions verified. Now — a few more values.

⟫ 2/7 · Agent name?
  📍 Your choice — short, letters+numbers only, visible in Teams (e.g., "ContosoHelper")

⟫ 3/7 · Manager email?
  📍 Contact email for agent registration — typically yours or your manager's

⟫ 4/7 · Your email for testing?
  📍 Where agent sends test emails in §2 — use your own inbox

⟫ 5/7 · Global Admin UPN?
  📍 Account with Global Admin rights (e.g., admin@contoso.onmicrosoft.com)

⟫ 6/7 · Foundry model deployment name?
  📍 ai.azure.com → your project → Models + Endpoints → the deployment name shown next to the model
...
⟫ 7/7 · Foundry model endpoint URL?
  ...
  (e.g., "https://your-resource.openai.azure.com/" or the serverless endpoint URL)
```
...
## §0 · The First Stone — Clone the Sample Repository

📖 The Agent365-Samples repository contains official Microsoft sample agents in Python, .NET, and Node.js. We'll use the Python sample — a fully functional agent with Outlook, Word, and other MCP tool integrations built in. Cloning this gives you a working codebase to configure and deploy.
...
| Field | Value |
|-------|-------|
| Source | Deploy Sample Agent App (A365).pdf + A365 Workshop Pre-Requisites Guide.pdf |
| Architect of the Path | V.K. |
| Version | 1.2 — 07 Apr 2026 |
